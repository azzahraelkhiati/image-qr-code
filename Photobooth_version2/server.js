...
function getLatestImageFromDir() {
    const files = fs.readdirSync(targetDir);
    
    const pngFiles = files.filter(file => file.toLowerCase().endsWith('.png'));
    
    if (pngFiles.length === 0) {
        throw new Error('Aucune image PNG trouvée dans le dossier');
    }
    
    const latestFile = pngFiles
        .map(filename => ({
            name: filename,
            time: fs.statSync(path.join(targetDir, filename)).mtime.getTime()
        }))
        .sort((a, b) => b.time - a.time)[0].name;
    
    return path.join(targetDir, latestFile);
}
...
function handleStableDiffusion(scriptPath, newFileName, req, res) {
    ...
    try {
        const latestImage = getLatestImageFromDir();
        console.log(`Utilisation de l'image : ${latestImage}`);
